...
def load_model(adapter_name:str) -> Tuple[PreTrainedModel, PreTrainedTokenizer]:

  # get base_model_name
  adapter_config = PeftConfigMixin.from_pretrained(adapter_name)
  base_model_name = adapter_config.base_model_name_or_path
  adapter_class_name = adapter_config.peft_type.name
  print(f"{adapter_name} = {base_model_name} with {adapter_class_name}")

  # check validity
  if base_model_name.lower() not in [
      "vaiv/gem2-llamion-14b-base",
      "vaiv/gem2-llamion-14b-chat",
      "vaiv/gem2-llamion-14b-longchat"]:
    raise ValueError(f"Invalid base model: {base_model_name}")

  if adapter_class_name not in ["LORA", "IA3", "BOFT", "VERA"]:
    raise ValueError(f"Invalid adapter class: {adapter_class_name}")
  ...
  model = AutoModelForCausalLM.from_pretrained(
      ...
      torch_dtype="auto",
      device_map="auto")
  num_base_params = model.num_parameters() # GeM2-Llamion-14B: 14_498_288_640
  model = PeftModel.from_pretrained(model, adapter_name)
  num_adapter_params = model.num_parameters() - num_base_params
  
  if num_adapter_params / num_base_params > 0.05:
    raise ValueError(
        f"Exceed the param limit: {num_adapter_params}" \
        f"({num_adapter_params * 100 / num_base_params:.2f}%)")
...
def evaluate_free_generation(
  ...
  llm = VLLM(
      ...
      tensor_parallel_size=args.num_gpus  # TODO
      )
...
def evaluate_adapter(args):
  ...
  model_path = f".merged/{args.adapter_name}"
  if not os.path.exists(model_path):
    model, tokenizer = load_model(adapter_name=args.adapter_name)
...
if __name__ == "__main__":
  ...
  parser.add_argument("--adapter_name", required=True)  # YOUR_ADAPTER_NAME
